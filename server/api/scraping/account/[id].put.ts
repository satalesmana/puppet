import { ScrapingAccount, allowType } from '~/server/models/ScrapingAccount.model'
...
export default defineEventHandler(async event => {
  try{
    ...
    const { modifiedCount,matchedCount }  = await ScrapingAccount.updateOne({_id:params?.id},{
      name : account,
      email : email,
      password : password,
      type: type,
